...
namespace hdc {
	...
	bool Argument::Parse(int argc, char *argv[]) {
		if (argc > 1) {
            ...
            if (s == "--console-with-cmd") {
				...
				if (argc > 2) {
					printf("%s\n", (utils::Aes::CryptoHex(argv[2], hdc::GetDataSecuretKey())).c_str());
				}
				else {
			...
			//	printf("%s\n", utils::Aes::HexDecrypto(argv[2], hdc::GetDataSecuretKey()).c_str());
			//	return true;
			//}
			else if (s == "--sm3" ) {
				if (argc > 2) {
				...
				std::string sk_hash = utils::String::BinToHexString(hdc::HashWrapper::Crypto(hdc::GetDataSecuretKey())).substr(0, 2);
#ifdef SVNVERSION
				printf("%s;%u;secure:%s;git:" SVNVERSION "\n", General::hdc_VERSION, General::LEDGER_VERSION, sk_hash.c_str());
#else
				printf("%s;%u;secure:%s\n", General::hdc_VERSION, General::LEDGER_VERSION, sk_hash.c_str());
#endif 
				return true;
				...
				result["private_key_aes"] = utils::Aes::CryptoHex(private_key, hdc::GetDataSecuretKey());
	...
	void Argument::ShowNodeId(int argc, char *argv[]) {
		...
		hdc::PrivateKey private_key(utils::Aes::HexDecrypto(argv[2], hdc::GetDataSecuretKey()));
